...
import { CrossChainQRCodeDisplay } from '../../features/create-order/components/CrossChainQRCodeDisplay';
    ...
    useEffect(() => {
        if (order) {
            ...
            if (firstValidChain) {
                ...
                setActiveTab('cross-chain-order-swap');
            }
        }
    }, [order]);
        ...
        <div className="max-w-4xl mx-auto bg-white dark:bg-primary-950 p-8 rounded-2xl shadow-lg shadow-dynamic">
            ...
            <div className="sticky top-0 z-10 bg-white dark:bg-primary-950 border-b border-gray-200 dark:border-gray-800">
                <nav className="-mb-px flex space-x-4 overflow-x-auto" aria-label="Payment Tabs">
                    ...
                    {crossChainUrl && (
                        <button
                            onClick={() => setActiveTab('cross-chain-order-swap')}
                            className={`whitespace-nowrap py-3 px-1 border-b-2 font-medium text-sm ${activeTab === 'cross-chain-order-swap' ? 'border-b-2 border-gray-500 text-gray-600 dark:text-gray-400' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-200'}`}
                        >
            ...
            <div className="py-6">
                ...
                {activeTab === 'cross-chain-order-swap' && crossChainUrl && (
                    // Pass a no-op function for onBackToOrderDetails as this is a standalone page
                    <CrossChainQRCodeDisplay order={{ ...order, crossChainUrl }} onBackToOrderDetails={() => { /* no-op */ }} />
                )}
                {activeTab === 'cross-chain-order-swap' && !crossChainUrl && (
