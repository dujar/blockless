...
export const Combined1InchSwapQRCode = ({ order, onBackToOrderDetails }: Combined1InchSwapQRCodeProps) => {
    ...
    if (!shareUrl) {
        return <p className="text-sm text-gray-500 dark:text-gray-400">No valid cross-chain swap URL could be generated. Ensure at least one token is configured.</p>;
    }
        ...
        <div className={isFullScreenQR ? 'fixed inset-0 z-50 bg-white dark:bg-primary-950 p-4 flex flex-col justify-between' : ''} onClick={() => isFullScreenQR && setIsFullScreenQR(false)}>
            {isFullScreenQR && (
                <div className="text-center mb-4" onClick={(e) => e.stopPropagation()}>
                    <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">Scan to pay from any chain / token</h3>
